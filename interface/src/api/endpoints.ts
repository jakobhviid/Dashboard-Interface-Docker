...
const port = (window as any)._env_ === undefined ? 5000 : (window as any)._env_.DASHBOARDI_SOCKET_SERVER_PORT;
export const SOCKET_SERVER_ENDPOINT =
  (window as any)._env_ === undefined ? `http://127.0.0.1:${port}` : `${(window as any)._env_.DASHBOARDI_HOST_DNS}:${port}`;
