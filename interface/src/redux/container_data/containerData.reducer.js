  ...
  switch (action.type) {
    ...
    case containerActionTypes.RENAME_CONTAINER_SUCCESS:
      return produce(state, (nextState) => {
        ...
        for (const [index, container] of state.overviewData[server]["containers"].entries()) {
          if (container.id === updatedContainer.id) {
            nextState.overviewData[server]["containers"][index] = updatedContainer;
          }
        }
      });
    ...
    case containerActionTypes.START_OR_STOP_CONTAINER_SUCCESS:
      return produce(state, (nextState) => {
        ...
        updatedContainer.state = updatedContainer.state.includes("exited") ? "running" : "exited";

        updatedContainer.statis = updatedContainer.state.includes("exited") ? "Exited a few seconds ago" : "Up a few seconds";
    ...
    case containerActionTypes.REMOVE_CONTAINER_SUCCESS:
      return produce(state, (nextState) => {
        ...
        if (indexOfContainer !== null) {
          nextState.overviewData[server]["containers"].splice(indexOfContainer, 1);
        }
      });
